...
thread_alarm_clock_sleep(void){
	...
	if(current != idle_thread){
		list_insert_ordered(&wakeup_list, &current->elem, (list_less_func *) &compare_ticks, NULL);
		thread_block();
		schedule();
	}
	intr_set_level(old_level);
	thread_alarm_clock_wakeup();
}

void
thread_alarm_clock_wakeup(void){
	struct thread *t;
	enum intr_level old_level;
